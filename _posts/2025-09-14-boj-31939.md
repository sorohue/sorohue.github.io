...
\\[ {1 \over \pi R^2} \int _0 ^ R \int_0 ^ {2\pi} r\sum a_k r^k e^{ik\theta} \sum \overline{a_k} r^k e^{-ik\theta}\ d\theta dr \\]
로 나타낼 수 있습니다.
...
\\[ {1 \over \pi R^2} \int_0 ^ R \int_0 ^ {2\pi} \sum \vert a_k\vert ^2 r^{2k+1}\ d\theta dr \\]
\\[ = {2 \over R^2} \int_0 ^ R \sum \vert a_k\vert ^2 r^{2k+1}\ dr = \sum { \vert a_k\vert ^2 R^{2k} \over k+1} \\]
로 정리됩니다.
